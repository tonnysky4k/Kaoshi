...
//	点击添加分类弹出
	...
	$(".sp-kaogz").click(function(){
    		$(".layuiBg").css({
    			display:"block",height:$(document).height()
    		});
    		$(".ksGzbox").css({
    			left:($("body").width()-$(".ksGzbox").width())/2-20+"px",
    			top:($(window).height()-$(".ksGzbox").height())/2+$(window).scrollTop()+"px",
    			display:"block"
    		});
    	});
	//修改弹出层
	...
	$(".layerClose, .layuiBg ,.saveBtn ,.cancelBtn").click(function(){
		$(".layuiBg ,.imgXgbox ,.addFeileibox ,.addRenZbox ,.ksGzbox,.Columnbox ,.ColumnXgbox ,.xcXgBox").fadeOut();
	});
